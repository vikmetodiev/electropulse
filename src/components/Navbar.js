...
import { TabContent, Nav, NavItem, NavLink, NavbarToggler, Collapse, Navbar } from 'reactstrap';
...
import { faEnvelopeOpenText, faSun, faMoon, faGlobeEurope } from "@fortawesome/free-solid-svg-icons"
    ...
    if (window.localStorage.getItem('language') === "bg") {
        return <NavItem>
            <NavLink
                ...
                onClick={() => { i18n.changeLanguage('en'); localStorage.setItem('language', 'en'); isMobile ? props.toggle() : i18n.changeLanguage('en') }}
            >
                <FontAwesomeIcon icon={faGlobeEurope} className="nav-icons" style={{ fontSize: 20 }} /> {t('navbar.language')}
            </NavLink>
        </NavItem>
    } else {
        return <NavItem>
                ...
                <div className={scrollTop > 0 ? "header" : "not-scrolled-header"}>
                    <Navbar className="background-0" light expand="md">
                        ...
                        <Collapse isOpen={this.state.isOpen} navbar>
                            <Nav navbar className="nav-blue-tabs border-bottom-0 mb-1 nav-positioning">
